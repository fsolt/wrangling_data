...
Like 'janitor work' itself, data-entry errors have thus far gained little attention among political scientists.
**The rest of this section still needs revised.**
...
Rather than merely a single replication of a difficult-to-detect phenomenon, we hope readers will also consider this work as a contribution to the growing "open science" movement to build more reliable and transparent research both in and beyond social science [@Christensen2019; @EngzellRohrer2021].
...
The Standardized World Income Inequality Database (SWIID) is a long-running project that seeks to provide harmonized inequality statistics for the broadest possible coverage of countries and years [@Solt2009; @Solt2015; @Solt2016; @Solt2020].
As of its most recent update at the time of this writing, its source data consists of some 27,000 observations of the Gini coefficient in various countries and years, collected from over 400 separate sources including international organizations, national statistics bureaus, and academic studies.^[
...
To avoid potential data-entry errors, as well as to ensure that the updates and revisions that are frequent in many of these sources are automatically incorporated as they become available, the process of collecting the source data is automated to the greatest extent practicable [@Solt2020, 1184-1185].^[
The R code for this automated data collection can be viewed here: <https://github.com/fsolt/swiid/blob/master/R/data_setup.R>.]

Most international organizations and a few national statistical bureaus use application programming interfaces (APIs) that facilitate downloading their data; the R community has often built packages using these APIs to make the task even easier [see @Blondel2018; @Magnusson2014; @Lahti2017; @Lugo2017; @Wickham2018].
...
Some sources---including most academic articles---are behind paywalls, making reproducibility particularly challenging.^[
...
All of the entries in this spreadsheet are entered twice into two separate spreadsheets, most often by two separate researchers.
This practice allows for automated cross-checks that increase the chance that errors are identified and corrected [see @BarchardPace2011].
