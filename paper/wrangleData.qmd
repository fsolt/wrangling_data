...
In short, incomplete/inaccurate data entry, the absence of reproducibility, and untrackable human discretion in manual janitor work have collectively become major obstacles on the way to data harmonization, yet these challenge has received surprisingly little attention.
...
Advancing this line of work, DCPO not only provides latent variable measurements but also automated and reproducible data collection [@Solt2020], which has been applied in a complete pipeline for a variety of topics such as gender egalitarianism [@Woo2023], political interest [@Hu2025a], and support for gay rights [@Woo2025], among other aspects of public opinion and open it freely for global researchers (see the data available at <https://dcpo.org/>).^[
  Note that harmonizing aggregate data, as in SWIID, and survey data, as in DCPO, involves different specific challenges. 
...
In the DCPO case, nce the research team manually codes the question numbers of relevant items, response scales, survey weights, project names, fieldwork waves, and dates from the raw survey files and accompanying documentation, data entry can be automated through the R-based software, `DCPOtools` [@SoltEtAl2018a]. 
...
Nevertheless, it enables efficient integration of updates and revisions whenever data collection program is reran.
...
For data that one has to enter manually, the teamwork is crucial.
...
Researchers are encouraged to pilot the automation on a small, diverse sample or conduct spot checks to verify the quality of automated data entry.
...
Nevertheless, issue of researcher degrees of freedom indicates that current standards of openness remain insufficient [see @HuTaiSolt2025].
In the context of data harmonization, we advocate for what we call a "second-order" opening. 
The "first-order" openness refers to sharing code and outputs from the analysis stage, and second-order openness involves making transparent the data generation process (DGP)—including data collection, cleaning, wrangling, and harmonization decisions.
...
Group B, which used the same design but processed the data themselves, produced the highest variation in results.
...
Along with a clearly conceptualized theoretical framework, researchers can simply share their programming scripts for data downloading, formatting, and wrangling, and thereby ensure that the full pipeline is documented and reproducible.^[
  @OserZur2025 also pointed out that the expertise of programming can also affect the reproducibility of the data wrangling process.
]
...
tt(ls_packages$download, 
  ...
  caption = "Example packages for downloading data with API\\label{tbl-download}") |> 
     style_tt(theme = "latex", tabularray_inner = "stretch=0",
      multipage = TRUE) |> 
  style_tt(fontsize = 0.7)
