...
The wrangling step plays a critical role in determining data quality for harmonization, especially given the growing volume and diversity of available sources and the need for transparent, well-documented cleaning [@Kolczynska2022].
...
This TAO covers the three phases of data wrangling: data selection and collection, data entry, and what we term second-order opening—the transparent documentation of the data generation process (DGP).
...
1. To reduce the manual entry errors in order to improve the accuracy of both harmonized and analytic datasets; 
1. To incorporate as much available data as possible to provide a solid foundation for constructing comparable data and increasing the generalizability of inferences;^[
...
Advancing this line of work, DCPO not only provides latent variable measurements but also automated and reproducible data collection [@Solt2020], which has been applied in a complete pipeline for a variety of topics including gender egalitarianism [@WooEtAl2023], political interest [@HuSolt2025], and support for gay rights [@WooEtAl2025] among other aspects of public opinion and open it freely for global researchers (see the data available at <https://dcpo.org/>).^[
  Note that harmonizing aggregate data, as in the SWIID, and survey data, as in DCPO, involves different specific challenges. 
  ...
  We use the two cases to illustrate how our TAO routine can benefit diverse data harmonization missions.
]
