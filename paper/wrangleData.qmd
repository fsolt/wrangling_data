...
To avoid data-entry errors and ensure that updates and revisions are automatically incorporated as they become available, for the past decade the process of collecting the source data has been automated to the greatest extent practicable [@Solt2020, 1184-1185].^[
...
The upshot of this process is that the SWIID's harmonized estimates of income inequality are reliable, frequently updated, and employed around the world by international organizations, central banks, and researchers in academia and beyond.


# Wrangling Public Opinion Data for Harmonization

Scholarship on comparative public opinion only rarely benefits from relevant items asked annually by the same survey in many countries.^[
Examples of such fortunate works include @Norris2011 [, 70-77] on trust in government and @Hagemann2017 on attitudes toward European integration; it is telling perhaps that both are based on the extremely rich Eurobarometer surveys and hence limited to European countries.]
To address the lack of cross-national and longitudinal data on many topics, a number of works have presented latent variable models that harmonize available but incomparable survey items [see @Claassen2019; @Caughey2019; @McGann2019; @Solt2020c; @Kolczynska2024].
...
Multiple collaborators go through the entire process separately, and the resulting spreadsheets are compared to catch the omnipresent data-entry errors.
...
For three survey items contributing observations to a total of 135 country-years, the differences were found to result from deviations from the original study's coding rules: for two items the median category was mistakenly also counted as a democracy-supporting response, and for one an above-median category was not (for details, see the online appendix).
...
Any one of these mistakes should give pause to any researchers tempted to meet the  challenge of 'janitor work' to harmonization research by hand entering data.
Correcting all of them is sufficient to undermine the results published in @Claassen2020b on the sources of democratic support, as shown in the online appendix.


# Discussion

Data-entry errors are an especially pernicious threat to data-intensive research such as harmonization projects.
...
Our recommendations---to maximize automization, use the double-entry method, and embrace teamwork---take effort [cf., @EngzellRohrer2021].
...
Moreover, as the example of `DCPOtools` demonstrates, many common janitor-work chores already have been packaged as open-source software to make researchers' task easier and more straightforward. 
...
And while the double-entry method is labor intensive, experiments have shown that it reduces error rates by thirty-fold even when done immediately after the initial collection and by the same person [@Barchard2011, 1837]; it is worth the extra effort.
...
# Deviations from the Stated Coding Rules in 
...
Although this may be interpreted as an exercise of researcher judgment as to what constitutes a democracy-supporting response rather than a data-entry error [see @Hu2025], examination of similar answers to similar questions confirms that similarly lukewarm responses at and below the median response category  (e.g., in the Arab Barometer, that democracy was "somewhat appropriate" for the country) were coded as not supportive.]
