...
thanks: "Yue Hu appreciates the funding support from the National Natural Science Foundation of China (72374116) and Tsinghua University Initiative Scientific Research Program (2024THZWJC01)."
...
title: "Wrangling Pre-harmonized Data: Best Practices Based on Harmonizing Income Inequality and Aggregate Public Opinion Measures"

subtitle: "Preliminary version. No cycling without permission."
...
suggests best practices for helping scholars avoid errors in this often-tedious work. To provide illustrations of these best practices, the article uses the examples of harmonizing procedures used to produce the Standardized World Income Inequality Database (SWIID), a widely used database that uses Gini indices from multiple institutions (e.g., UN, Eurostat, OECD, Luxembourg Income Study) to create comparable estimates; and the Dynamic Comparative Public Opinion (DCPO) project, which creates a workflow for harmonizing aggregate public opinion data."
...
In @fig-comparison, we compare the percentage of respondents to give a democracy-supporting response in the publication spreadsheet with the percentage we found using our automated process of wrangling these same data.
...
#| fig-cap: "Comparing Democracy-Supporting Responses in the Publication Data and the Corrected Data.The x-axis represents the corrected data, and the y-axis represents the publication data. The points are colored by the item family and shaped by the project. The dotted line represents the line of equality. The plot shows a strong positive correlation between the two variables, with most points falling above the line of equality."
#| fig-align: "center"
...
## Workflow with a Higher Reliability
...
df_toy <- surveys_data |> 
  filter(archive %in% c("icpsr", "pew")) |> 
  group_by(archive) |> 
  slice_head(n = 1) |> 
  ungroup() |> 
  select(survey, file_id)

df_raw <- get_surveys(surveys_data)
```
