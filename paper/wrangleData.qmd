...
Although the pernicious consequences of data-entry errors are easily grasped in everyday contexts---@Haegemens2019 [, 1] collects examples of misrouted financial transactions and airline flights---they have thus far gained little attention in political science, even among those working to harmonize large quantities of data.
...
Second, _use the double-entry method_: when manual data entry cannot be avoided, each entry should be made twice.
...
All new observations are entered twice into separate spreadsheets, most often by two different researchers.
This practice allows for automated cross-checks of the newly entered data that increase the chances that errors are identified and corrected [see @Barchard2011].

The upshot the SWIID's highly automated process for collecting source data is that its harmonized estimates of income inequality are reliable, frequently updated, and employed by international organizations, central banks, and researchers working around the world.


# Wrangling Survey Data for Harmonization
