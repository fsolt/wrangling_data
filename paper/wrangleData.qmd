...
abstract: > 
  This article focuses on a preliminary step in any ex-post data harmonization project—
  wrangling the pre-harmonized data—and discusses key considerations.
...
Data harmonization projects---and a growing volume of other political science research---can be characterized as data science: they employ large quantities of data, often drawn from a large number of different sources.
...
Data wrangling is, however, critically important to scientific inquiry, and errors that arise during this process can undermine our conclusions.
...
# Avoiding Data Entry Errors in Income Inequality Data
...
To avoid potential data-entry errors, as well as to ensure that the updates and revisions that are frequent in many of these sources are automatically incorporated as they become available, the process of collecting these source data is automated to the greatest extent practicable [@Solt2020, 1184-1185].^[
The R code for this automated data collection can be viewed here: <https://github.com/fsolt/swiid/blob/master/R/data_setup.R>.]
Most international organizations and a few national statistical bureaus use application programming interfaces (APIs) that facilitate downloading their data; the R community has often built packages using these APIs to make the task even easier [see @Lahti2017;  @Wickham2017].
