...
In addition, manual wrangling make a full reproducibility of research pipeline more difficult and undermine the transparency [@Liu2019], especially when raw data comes from heterogeneous sources and has been processed using various software environments over time.
For example, older survey files stored in SPSS's ASCII or portable formats often require extensive restructuring before they can be merged with newer datasets.
The undocumented transformations can complicate reproducibility.
...
In this article, we provide a practical routine (a "TAO") that takes advantage of automatic programming and teamwork to reduce such data-entry errors and improve the reproducibility and transparency of the wrangling process for researchers and reviewers to check the errors.
This TAO covers the three phases of data wrangling: data collection/selection, data entry, and opening.
...
<<<<<<< HEAD
Along this line, DCPO not only provides latent variable measurements but also automatized and reproducible data collection [@Solt2020], which has been applied in a complete pipeline for a variety of topics such as gender egalitarianism [@Woo2023], political interest [@Hu2025a], and support for gay rights [@Woo2025], among other aspects of public opinion and open it freely for global researchers (see the data available at <https://dcpo.org/>).
=======
In line with this work, DCPO not only generates latent variable estimates but also provides automatized and reproducible data collection tools [@Solt2020].
It has been applied in a complete pipeline on topics such as gender egalitarianism, political interest, and support for gay rights, among other public opinion topics, and is freely available for researchers (see more updated data collections at <https://dcpo.org/>).
>>>>>>> refs/remotes/origin/main
...
=======
Large scale of data selection and cleaning is almost always tedious, as something to be delegated to research assistants, to someone---indeed anyone, but usually research assistants (RA)---else.
This manual procedure is susceptible to errors.
...
=======
A deliberative set requires the members in a research team---regardless several coauthors or a primary author with one or two RAs---to have a clear and coherent understanding of the research questions and associated data goals.
These understandings will help the team members identify the right data to collect and discover extra useful data sources that are not in the initial plan.

In the SWIID program, for example, we explain to research assistants that the project aims to generate comparable statistics of country-level economic inequality.
We provide a list of sources-primarily from national statistic bureaus-as a starting point, but also note that updated statistics might be available from academic publications or other documents.
Research assistants are encouraged to include such sources, provided they document valid links.


Ensuring that team members understand how the data will be used later is also important, as it helps them develop a better sense of which data is analytically useful and prepare for the demands of later data-entry stages.
In the SWIID project, we instruct RAs to record inequality statistics in four formats: Gini index in disposable (post-tax, post-transfer) income, Gini in market (pre-tax, pre-transfer) income, absolute redistribution (market-income inequality minus net-income inequality), or relative redistribution (market-income inequality minus net-income inequality, divided by market-income inequality).
For later unification work, they are expected to record the values and document the methods used to generate them.

The SWIID project requires update for almost every year and we frequently hire new RAs.
Therefore, cross-checking is conducted on a rolling basis usually by new team members who review the previously collected data and update broken or outdatedlinks.
This is both a learning process and a means of improving data accuracy.
>>>>>>> refs/remotes/origin/main
...
Following the initial round of item selection and collection, the dural-entry section begins. 
...
Therefore, we organize multiple lab meetings throughout the data selection phase to share insights from each member’s coding work and ensure conceptual alignment across the team.
...
Both SWIID and DCPO projects enrolled team members from outside the U.S.
These members draw on their linguistic and cultural expertise to detect extra sources in non-English languages and improve the precision of the data selections.
...
The best solution is to automate the entry process taking the advantages of the programming languages and application programming interfaces (APIs) of the data source.
=======
...
When no API is available, the automation script downloads and reads any available spreadsheets.
...
One source of SWIID contains crucial information encoded in the typeface of its tables; this information would be lost if the tables were read directly into R.
...
```{r}
#| label: tbl-download
#| tbl-cap: Example packages for downloading data with API
...
```{r}
#| label: tbl-clean
#| tb-cap: Example packages for cleaning data with API
...
```{r}
#| label: tbl-transform
#| tbl-cap: Example packages for transforming data with API
