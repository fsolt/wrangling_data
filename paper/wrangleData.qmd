...
These challenges are amplified when raw data comes from heterogeneous sources and has been processed using various software environments over time.
...
Along this line, DCPO not only provides latent variable measurements but also automatized and reproducible data collection [@Solt2020], which has been applied in a complete pipeline for a variety of topics such as gender egalitarianism, political interest, and support for gay rights, among other aspects of public opinion and open it freely for global researchers (see more updated data collections at <https://dcpo.org/>).
...
(Check a more elaborate checklist in @sec-checklist.)
