...
@Barchard2011 found that "research assistants"(RA) assigned in an experiment to carefully enter data manually, even those instructed to double-check their entries against the original, had error rates approaching 1% in just a single roughly half-hour session.
The consequences of such errors can be pernicious.
@Haegemans2019 [p. 1] has demonstrated examples of misrouted financial transactions and airline flights.
Here we provide a more vivid one from political science to show how big difference merely data wrangling can make:
...
The routine includes three steps: team-based data selection, automate data entry, and "second-order" data opening.
...
1. To make the data wrangling process more smooth and traceable.

The routine decomposes a data-wrangling process into three steps: data selection, data entry, and data opening. 
In the following part, we will first address the challenges research may cope with in each step, and then provide some solutions and guides for them.

To illustrate the solutions in real research practices, we use two data harmonization projects as examples, SWIID and DCPO.
...
DCPO as a Bayesian-based latent variable method provides an efficient way for researchers to achieve cross-sectional time-series comparison among surveys conducted in different countries addressing ordinal scale of the latent measures and opinion polarization in the source data [@Solt2020c].



presented in  in particular has been employed to measure gender egalitarianism [@Woo2023], political interest [@Hu2024], and support for gay rights [@Woo2025], among other aspects of public opinion (see <https://dcpo.org/>).
...
## Step 3: "Second-Order" Data Opening

Those who are interested can access and explore these data on the web at <https://fsolt.org/swiid/swiid_source.html>.


covers the process from data collection to cleaning and reformatting. 


- Step 1: group with multiple members
    - Efficiency 
    - Double check
    - Aware of the logic and the purposes of the project
- Step 2: Collect and organize data
  - Challenges
      - The latest, best data
      - The human error
      - The human judgement
      - The transparency
  - Solutions
      - Automation
      - Traceable working sheet
      - Multiple researchers coding and cross-checking
  - Nonopinionative data: e.g., SWIID
  - Opinionative data: e.g., DCPO
- Step 3: open source
    - OSF 
    - Github
...
# Practical Suggestions of Data Wrangling

- A diagram
- Automation means more human factor involved
- Potential of AI
...
```{r regressionPlot, fig.cap= "", fig.width=7, fig.height=8}

#| label: regressionPlot
